        ...
        <div class="w-full p-4">
            ...
            <form action="{{ route('admin.pengguna.update', $pengguna->id) }}" method="POST">
                ...
                <div class="mb-4">
                    ...
                    <input class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="nama" type="text" name="nama" value="{{ $pengguna->nama }}" required>
                </div>

                <div class="mb-4">
                    <label class="block text-gray-700 text-sm font-bold mb-2" for="email">Email</label>
                    <input class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="email" type="email" name="email" value="{{ $pengguna->email }}" required>
                </div>

                {{-- Password opsional, kosongkan kalau tidak mau ganti --}}
                <div class="mb-4">
                    <label class="block text-gray-700 text-sm font-bold mb-2" for="password">Kata Sandi (Kosongkan jika tidak ingin mengubah)</label>
                ...
                <div class="mb-4">
                    ...
                    <select class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="peran" name="peran" required>
                        <option value="peminjam" {{ $pengguna->peran === 'peminjam' ? 'selected' : '' }}>Peminjam</option>
                        <option value="petugas" {{ $pengguna->peran === 'petugas' ? 'selected' : '' }}>Petugas</option>
                ...
                <div class="flex items-center justify-between">
                    ...
                    <a href="{{ route('admin.pengguna.index') }}" class="inline-block align-baseline font-bold text-sm text-blue-500 hover:text-blue-800">Batal</a>
                </div>
            </form>
        </div>
    </div>
