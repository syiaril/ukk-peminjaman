                    ...
                    <tr>
                        <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                            <div class="font-bold flex items-center gap-2">
                                {{ $p->pengguna->nama }}
                                @if($p->riwayat_telat > 0)
                                    <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-red-100 text-red-800 border border-red-200" title="Pernah terlambat atau ada denda">
                                        ⚠️ Berisiko
                                    </span>
                                @elseif($p->riwayat_aman > 0 && $p->riwayat_telat == 0)
                                    <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-green-100 text-green-800 border border-green-200" title="Selalu tepat waktu">
                                        ⭐ Aman
                                    </span>
                                @else
                                    <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-blue-100 text-blue-800 border border-blue-200" title="Belum memiliki riwayat peminjaman">
                                        🔵 Baru
                                    </span>
                                @endif
                            </div>
                            <div class="flex items-center gap-2 mt-1">
                                <span class="text-[10px] bg-gray-100 px-1 rounded text-gray-500">Aman: {{ $p->riwayat_aman }}</span>
                                <span class="text-[10px] bg-gray-100 px-1 rounded text-gray-500">Telat: {{ $p->riwayat_telat }}</span>
                            </div>
                            <div class="text-xs text-gray-400 mt-1">{{ $p->pengguna->email }}</div>
                        </td>
                        <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                            {{ $p->alat->nama_alat }}
                            @if($p->jumlah > 1)
